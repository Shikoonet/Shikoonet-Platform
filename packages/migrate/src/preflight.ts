...
export async function preflight(
  ...
  for (const table of [
    'user', 'invoice', 'Payment_report', 'service_other', 'product',
    'marzban_panel', 'card_number', 'card_assignment_leases', 'Discount',
    'DiscountSell', 'Giftcodeconsumed', 'reagent_report', 'Requestagent',
    'revenue_adjustment_log', 'wheel_list', 'support_message', 'departman',
    'help', 'app', 'channels', 'admin', 'setting', 'shopSetting', 'PaySetting',
  ]) {
  ...
  for (const table of [
    'devices', 'device_credentials', 'financial_accounts',
    'financial_account_identifiers', 'raw_sms_events', 'transaction_candidates',
    'payment_claims', 'reconciliation_matches', 'payment_cards', 'access_users',
    'audit_logs', 'integration_events', 'transaction_reviews',
    'transaction_detected_identifiers', 'transaction_account_assignments',
  ]) {
  ...
  if (dupCodes.length === 0 && crossCodes.length === 0) {
    ...
    add('NOTICE', 'duplicate discount codes',
  ...
  const badGifts = await mysqlRows<{ id: number; code: string; price: string | null }>(
    my, "SELECT id, code, price FROM Discount WHERE price IS NULL OR price = '' OR price = '0'",
  );
  for (const g of badGifts) {
    report.warn(`gift code "${g.code}" credits nothing (price = ${JSON.stringify(g.price)})`);
    add('NOTICE', 'empty gift code',
  ...
  const botCards = (
    await mysqlRows<{ cardnumber: string; namecard: string }>(
      my, 'SELECT cardnumber, namecard FROM card_number',
    )
  ...
  report.step(`bot ${botCards.length - badBot.length}/${botCards.length} pass Luhn, ` +
  ...
  const { rows: existing } = await pgc.query<{ users: string; payments: string }>(
  ...
  if (already > 0) {
    report.warn(`target already holds ${fmt(already)} users and ` +
  ...
  const [balances] = await mysqlRows<{ total: string; negatives: number }>(
    my, 'SELECT SUM(Balance) AS total, SUM(Balance < 0) AS negatives FROM user',
  );
  ...
  report.detail(`= ${fmt(BigInt(balances?.total ?? 0) * 10n)} IRR and ` +
  ...
  if (Number(balances?.negatives ?? 0) > 0) {
    const neg = await mysqlRows<{ id: string; username: string; Balance: string }>(
      my, 'SELECT id, username, Balance FROM user WHERE Balance < 0',
    );
