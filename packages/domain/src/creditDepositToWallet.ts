...
export async function creditDepositToWallet(
  ...
  return db.withSession(async (tx): Promise<CreditDepositResult> => {
    ...
    const credited = await tx
      ...
      .bind(args.userId, amountIrr, args.actorEmail, reason, depositWalletKey(args.transactionId))
      .run();
    ...
    return {
      ok: true,
      amountIrr,
      balanceIrr: Number(balance?.balance_irr ?? 0),
      notified: notified.meta.changes > 0,
    };
  });
}
