/**
 * Row shapes and SQL for the payment hub. No ORM — explicit SQL keeps the query
 * plan readable and the migrations the source of truth.
 *
 * Money: INTEGER IRR. Timestamps: epoch milliseconds (INTEGER).
 *
 * The database interface itself now lives in `@shikoo/db`, which implements it
 * on Postgres. It is re-exported here so that the ~40 files importing
 * `D1Database` from this package keep compiling untouched. The name is
 * historical: nothing in the hub ever imported @cloudflare/workers-types, which
 * is exactly why swapping the engine was an adapter rather than a rewrite.
 */
