...
--   3. A card handed to an invoice is out of the line for ten minutes, and for
--      as long as the customer says they paid and nobody has settled it. That
--      needs no column: the open `payments` row IS the lease (created when the
--      card is shown, closed by PAID / REJECTED / EXPIRED), and the picker reads
--      it. What it needs is an index to read it by card.
--
