...
-- A config's own volume bonus (0082, moved down from the service in 0081)
-- keeps the code's range: 0 ≤ p ≤ 100. Here, after the product exists, so the
-- refusal is the CHECK's and not a foreign key's — `assert_rejects` cannot
-- tell the two apart.
SELECT assert_rejects($$
  INSERT INTO product_plans (product_id, name, price_irr, bonus_percent)
       VALUES ((SELECT id FROM products WHERE code = '__inv-product'), '__inv-bonus-101', 1000, 101)
$$, 'a config cannot give more than +100% volume');
SELECT assert_rejects($$
  INSERT INTO product_plans (product_id, name, price_irr, bonus_percent)
       VALUES ((SELECT id FROM products WHERE code = '__inv-product'), '__inv-bonus-neg', 1000, -1)
$$, 'a config cannot give negative volume');
