...
| تست تضمین‌ها | `migrations/verify_invariants.sql` — **۱۸/۱۸ سبز**، ۲۰۲۶-۰۸-۱۲، روی Postgres 16.14 |
...
یکی از اختلاف‌ها **فقط یک رقم** است → به `BUGS-FOR-ADMIN.md` اضافه شد.
...
## سه سوال که جواب تو را می‌خواهد

**۱. آن دو کارت.** Hub کارت `5054161716277062` را دارد، ربات `5054161706277062` را — یک رقم فرق،
همان صاحب («هنرمندنسب»)، همان بانک گردشگری. کارت ربات ۵ پرداخت واقعی گرفته که ۴ تایش paid شده.
الگوی بقیهٔ کارت‌های همان بانک (`…۱۷ ۰ ۶…`) می‌گوید نسخهٔ Hub غلط تایپ شده. **تایید می‌کنی؟**
تا وقتی تایید نشود مهاجرت این دو را ادغام نمی‌کند.

**۲. آن مانده منفی.** کاربر `314985971` مانده `-5,940,000` تومان دارد. مهاجرت **عیناً** منتقلش می‌کند
مگر بگویی چیز دیگری. (پیشنهاد من: عیناً منتقل شود و بعداً با یک ردیف اصلاحی در دفتر کل حل شود،
تا خودِ مهاجرت هیچ‌وقت پول را دست‌کاری نکند.)

**۳. اکسپورت تازهٔ D1.** آخرین اکسپورتی که داریم مال ۲۰۲۶-۰۸-۱۰ است و دامپ MySQL مال ۰۸-۱۱.
برای مهاجرت واقعی به یک اکسپورت هم‌زمان از هر دو نیاز داریم. الان لازم نیست — با همین‌ها اسکریپت
نوشته و تست می‌شود — ولی موقع کات‌اوور باید تازه گرفته شود.
