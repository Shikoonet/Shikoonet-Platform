...
import { WAITING_TIMEOUT_MS } from '@hub/contracts';
import {
...
describe('10-minute waiting / Suspected Fake projection', () => {
  ...
  it('6 transaction arrives while waiting → AUTO_VERIFIED when unique in window', () => {
    const d = decide(claim(), [tx({ bankTimestamp: RECEIPT_MS + 15_000 })], [], RECEIPT_MS + 20_000);
    expect(d.decision).toBe('AUTO_VERIFY');
  });
  ...
  it('7 ambiguous transactions after 10m → NEEDS_REVIEW not Suspected Fake', () => {
    const d = decide(
      claim(),
      [tx({ id: 't1', bankTimestamp: RECEIPT_MS + 10_000 }), tx({ id: 't2', bankTimestamp: RECEIPT_MS + 20_000 })],
      [],
  ...
  it('8 outside 5m transaction after 10m → OUTSIDE_AUTO_MATCH_WINDOW not Suspected Fake', () => {
    const d = decide(claim(), [tx({ bankTimestamp: RECEIPT_MS + 6 * 60_000 })], [], RECEIPT_MS + WAITING_TIMEOUT_MS + 60_000);
    expect(d.decision).toBe('SUGGEST');
    expect(d.reason).toBe('OUTSIDE_AUTO_MATCH_WINDOW');
  });

  it('9 amount mismatch evidence → NEEDS_REVIEW not automatic Fake', () => {
    const d = decide(claim(), [tx({ amountIrr: 1_100_000, bankTimestamp: RECEIPT_MS + 10_000 })], [], RECEIPT_MS + WAITING_TIMEOUT_MS + 60_000);
    expect(d.decision).toBe('SUGGEST');
  ...
  it('11 late SMS after Suspected Fake classification → moves to Needs Review state', () => {
    ...
    const afterLateSms = decide(c, [tx({ bankTimestamp: RECEIPT_MS + 6 * 60_000 })], [], RECEIPT_MS + WAITING_TIMEOUT_MS + 120_000);
  ...
  it('never assigns FAKE_RECEIPT automatically', () => {
    const scenarios = [
      ...
      decide(claim(), [tx({ bankTimestamp: RECEIPT_MS + 120_000 })], [], RECEIPT_MS + WAITING_TIMEOUT_MS + 1_000),
      decide(claim({ cardMappingCount: 0, targetFinancialAccountId: null }), [], [], RECEIPT_MS + WAITING_TIMEOUT_MS + 1_000),
    ];
