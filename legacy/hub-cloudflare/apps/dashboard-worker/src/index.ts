...
import { parseSms, normalizeText } from '@hub/sms-parser';
import {
  generateApiToken,
  sha256Hex,
  tokenPrefix,
...
// Re-export the ambient D1Database type alias for use inside the file.
type DB = D1Database;

// ponytail: ambient Cloudflare D1Database is structurally compatible with
// @hub/database D1Database (our shape is a subset). Cast at the boundary.
function domainDb(db: DB): DomainD1Database {
  return db as unknown as DomainD1Database;
}

interface AppBindings {
  Bindings: Env;
  Variables: { identity: { email: string; role: import('@hub/contracts').AccessRole } };
}
...
// @hub/domain. Illegal transitions return 409 with a typed error. The
...
app.post('/api/v1/match/approve', async (c) => {
  ...
  const tx = await c.env.DB.prepare(
    ...
    .first<{
      id: string;
      status: import('@hub/contracts').TransactionStatus;
      direction: 'CREDIT' | 'DEBIT' | 'UNKNOWN';
  ...
  const match = await c.env.DB.prepare(`SELECT * FROM reconciliation_matches WHERE id = ?1`)
    ...
    .first<{
      ...
      transaction_candidate_id: string;
      payment_claim_id: string;
      status: import('@hub/contracts').MatchStatus;
    }>();
  ...
  const claim = await c.env.DB.prepare(
    ...
    .first<{ status: import('@hub/contracts').ClaimStatus; source_system: string }>();
  if (claim) {
    try {
