...
import SHA15 from '../../migrations/0015_purchase_type.sql?raw';
import {
  app,
  type Env,
} from '../src/index.js';

// Production schema = 0001..0014. Dev schema adds 0015, which production has
// not been migrated to yet — that gap is exactly what ENABLE_PURCHASE_TYPE gates.
const SCHEMA_PROD_ONLY = [
  SHA, SHA2, SHA3, SHA4, SHA5, SHA6, SHA7, SHA8, SHA9, SHA10, SHA11, SHA12, SHA13, SHA14,
]
  .map((s) =>
    s
      .replace(/^\s*--[^\n]*\n/gm, '')
...
const SCHEMA_DEV = SCHEMA_PROD_ONLY + '\n\n' + SHA15;
...
async function applySchema(sql: string): Promise<void> {
  for (const stmt of splitStatements(sql)) {
    try {
      await (baseEnv as unknown as Env).DB.prepare(stmt).run();
    } catch (err) {
...
describe('DEV — purchase_type feature flag', () => {
  ...
  describe('Date filter', () => {
     ...
     * production behavior.
     */
    const TEHRAN_TODAY_START = Date.parse('2026-08-10T03:30:00Z');
    const TEHRAN_TODAY_END = Date.parse('2026-08-11T03:30:00Z');
    const TEHRAN_YESTERDAY_START = Date.parse('2026-08-09T03:30:00Z');
    const TEHRAN_DAY_BEFORE_START = Date.parse('2026-08-08T03:30:00Z');
    ...
    it('Yesterday: returns rows inside yesterday Tehran window', async () => {
      // Yesterday in Tehran for "now" = 2026-08-10T07:42Z = 2026-08-09 (Tehran).
      // Worker "day" range for 2026-08-09: start=2026-08-09T03:30Z, end=2026-08-10T03:30Z.
      await seedWithVerifiedAt(
        'yd-1',
  ...
  describe('FEATURE OFF — generated SQL must not reference dev-only columns', () => {
    it('omits purchase_type / operation_type when ENABLE_PURCHASE_TYPE=false', async () => {
      ...
      await applySchema(SCHEMA_PROD_ONLY);
      // Seed an access user.
      await e.DB.prepare(
        `INSERT OR IGNORE INTO access_users (id, email, role, active, created_at, updated_at)
         VALUES (?1, ?2, 'ADMIN', 1, ?3, ?3)`,
