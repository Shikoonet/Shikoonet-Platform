...
import { MIRZABOT_CLAIMS_PATH } from '@hub/contracts';
...
async function applySchema() {
  for (const stmt of splitStatements(SCHEMA)) {
    try {
      ...
      if (String(err).includes('already exists') || String(err).includes('duplicate column')) continue;
      throw err;
    }
  }
}
...
function domainDb() {
  return env.DB as unknown as import('@hub/domain').D1Database;
}
...
describe('PHASE 5 — concurrency and double-use', () => {
  it('auto and manual approval racing the same transaction: exactly one wins', async () => {
    ...
    const { verifyMirzabotClaim } = await import('@hub/domain');
