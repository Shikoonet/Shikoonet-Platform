...
import SHA11MIRZA from '../../migrations/0011_mirzabot_integration.sql?raw';
import SHA12CARD from '../../migrations/0012_claim_card_digits.sql?raw';

const SCHEMA = [SHA, SHA2, SHA3, SHA4, SHA5, SHA6, SHA7, SHA8, SHA9, SHA10, SHA11MIRZA, SHA12CARD]
  .map((s) =>
    s
      .replace(/^\s*--[^\n]*\n/gm, '')
...
async function seedDevice(args: { deviceCode?: string; apiKey?: string } = {}) {
  ...
  await env.DB.prepare(`DELETE FROM device_credentials WHERE device_id = ?1 AND token_prefix = ?2`)
    .bind(deviceId, apiKey.slice(0, 4))
    .run();
  await env.DB.prepare(
    ...
    .bind(credId, deviceId, await sha256Hex(apiKey), apiKey.slice(0, 4), now)
    .run();
