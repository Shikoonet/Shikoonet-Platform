...
import { z } from 'zod';
import { INGEST_PATH, MAX_BODY_BYTES } from '@hub/contracts';
...
import type { MirzabotClaimPayload } from '@hub/contracts';
...
// Minimal D1 binding interface used by `auth.ts`/`ingest.ts`. Re-exported
// from @hub/database but inlined here to avoid the worker type collision.
type D1Database = import('@hub/database').D1Database;
...
function mirzabotMatchOpts(env: Env) {
  return {
...
export default {
  ...
  async scheduled(_event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ctx.waitUntil(finalizeExpiredMirzabotWaits(env.DB, mirzabotMatchOpts(env)));
  },
};
