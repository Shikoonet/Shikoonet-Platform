...
function paymentHubOrderLockPath(string $orderId): string {
    $safe = preg_replace('/[^a-zA-Z0-9._-]/', '', $orderId);
    return __DIR__ . '/data/integration-events/orders/' . $safe . '.lock';
}

function DirectPayment($order_id, $image = 'images.jpg')
{
    // [HUB FIX] acquire per-order flock for cross-path serialization
    $orderLockPath = paymentHubOrderLockPath((string) $order_id);
    @mkdir(dirname($orderLockPath), 0755, true);
    $orderLockFp = fopen($orderLockPath, 'c');
    if ($orderLockFp === false) {
        return ['ok' => false, 'state' => 'lock_failed', 'retryable' => true];
    }
    flock($orderLockFp, LOCK_EX);
    // [HUB FIX] reload authoritative state AFTER acquiring lock
    $Payment_report = select("Payment_report", "*", "id_order", $order_id, "select");
    if (!$Payment_report) {
        flock($orderLockFp, LOCK_UN); fclose($orderLockFp);
        return ['ok' => false, 'state' => 'order_not_found', 'retryable' => false];
    }
    $steppay = explode("|", $Payment_report['id_invoice']);
    $hub_username = null;
    if (is_array($steppay) && isset($steppay[0]) && $steppay[0] === 'getconfigafterpay' && isset($steppay[1])) {
        $hub_username = $steppay[1];
    }
    $hub_invoice = null;
    if ($hub_username !== null) {
        $hub_invoice = select("invoice", "Status, id_invoice, Service_location, name_product, Volume, Service_time, price_product, username", "username", $hub_username, "select");
    }
    if ($hub_invoice && $hub_invoice['Status'] === 'active' && $Payment_report['payment_Status'] === 'paid') {
        flock($orderLockFp, LOCK_UN); fclose($orderLockFp);
        return ['ok' => true, 'state' => 'already_fulfilled'];
    }
    // [RENEWAL FIX] at-most-once guard for renewals. If a previous cron tick
    // claimed this order as 'processing' and crashed before finalizing, do NOT
    // mutate the remote panel again. Operator reconciliation required.
    if ($Payment_report['payment_Status'] === 'processing') {
        flock($orderLockFp, LOCK_UN); fclose($orderLockFp);
        return ['ok' => false, 'state' => 'reconciliation_required', 'retryable' => false];
    }
    // [RENEWAL FIX] generic already_fulfilled guard for ALL branches. The
    // existing hub-specific guard only fires for getconfigafterpay because
    // $hub_invoice is null for renewals. For renewals, payment_Status='paid'
    // alone is sufficient evidence that DirectPayment has already finalized
    // this order. Short-circuit to prevent any future repeat mutation.
    if ($Payment_report['payment_Status'] === 'paid') {
        flock($orderLockFp, LOCK_UN); fclose($orderLockFp);
        return ['ok' => true, 'state' => 'already_fulfilled'];
    }
    // [HUB FIX] structured-result accumulator for the locked section
    $hubResult = null;
    try {
    ...
    if ($steppay[0] == "getconfigafterpay") {
        ...
        if ($dataoutput['username'] == null) {
            // [HUB FIX] before refunding, check if the service ALREADY exists
            // from a previous partial-success attempt (delivery_failed retry).
            $hubRecheck = $ManagePanel->DataUser($marzban_list_get['name_panel'], $username_ac);
            if (is_array($hubRecheck) && in_array($hubRecheck['status'] ?? null, ['active', 'disabled'], true)) {
                // Adopt the existing user; skip refund.
                $dataoutput = $hubRecheck;
            } else {
                ...
                $hubResult = ['ok' => false, 'state' => 'provisioning_failed', 'retryable' => false];
                return;
            }
        }
        ...
        $hubDeliveryResult = sendMessageService($marzban_list_get, $dataoutput['configs'], $output_config_link, $dataoutput['username'], $Shoppinginfo, $textcreatuser, $get_invoice['id_invoice'], $get_invoice['id_user'], $image);
        // [HUB FIX] Telegram delivery must succeed before any bookkeeping
        if (!is_array($hubDeliveryResult) || empty($hubDeliveryResult['ok'])) {
            $hubResult = ['ok' => false, 'state' => 'delivery_failed', 'service_provisioned' => true, 'retryable' => true];
            // do NOT refund; service exists. do NOT mark invoice active; do NOT mark Payment_report paid
            return;
        }
        ...
        // [HUB HOTFIX] authoritative finalization: mark Payment_report.paid only after
        // Telegram delivery has already returned ok=true (delivery_failed would have
        // early-returned at line 898-901) and invoice.Status='active' has been written.
        // This restores the post-lock idempotency guard's precondition so croncard
        // will short-circuit as already_fulfilled on subsequent ticks.
        ...
        // [RENEWAL FIX] claim at-most-once: marking 'processing' before remote
        // mutation. If this transaction crashes, the guard above will refuse
        // any further renewal mutation on this id_order until operator acts.
        update("Payment_report", "payment_Status", "processing", "id_order", $Payment_report['id_order']);
        $extend = $ManagePanel->extend($marzban_list_get['Methodextend'], $prodcut['Volume_constraint'], $prodcut['Service_time'], $nameloc['username'], $prodcut['code_product'], $marzban_list_get['code_panel']);
        if ($extend['status'] == false) {
            // [RENEWAL FIX] panel mutation failed without applying: revert to
            // 'waiting' so croncard may retry the renewal later.
            update("Payment_report", "payment_Status", "waiting", "id_order", $Payment_report['id_order']);
            $balance = $Balance_id['Balance'] + $Payment_report['price'];
            update("user", "Balance", $balance, "id", $Balance_id['id']);
            sendmessage($Balance_id['id'], $textbotlang['users']['sell']['errorConfig'], $keyboard, 'HTML');
        ...
        // [RENEWAL FIX] finalize: only NOW is Payment_report.paid authoritative.
        // After this point, the post-lock guard will short-circuit further calls.
        update("Payment_report", "payment_Status", "paid", "id_order", $Payment_report['id_order']);
        if ($Payment_report['Payment_Method'] == "cart to cart" or $Payment_report['Payment_Method'] == "arze digital offline") {
        ...
        $dateacc = date('Y/m/d H:i:s');
        $type = "extra_user";
        // [RENEWAL FIX] claim at-most-once before remote mutation.
        update("Payment_report", "payment_Status", "processing", "id_order", $Payment_report['id_order']);
        $extra_volume = $ManagePanel->extra_volume($nameloc['username'], $marzban_list_get['code_panel'], $volume);
        if ($extra_volume['status'] == false) {
            // [RENEWAL FIX] revert to 'waiting' on confirmed panel failure.
            update("Payment_report", "payment_Status", "waiting", "id_order", $Payment_report['id_order']);
        ...
        update("invoice", "Status", "active", "id_invoice", $nameloc['id_invoice']);
        // [RENEWAL FIX] finalize: at-most-once contract complete.
        update("Payment_report", "payment_Status", "paid", "id_order", $Payment_report['id_order']);
        ...
        // [HUB FIX] end locked section; finalize result
        $hubResult = is_array($hubResult) ? $hubResult : ['ok' => true, 'state' => 'fulfilled'];
    } finally {
        flock($orderLockFp, LOCK_UN);
        fclose($orderLockFp);
    }
    return $hubResult;
}
...
function sendMessageService($panel_info, $config, $sub_link, $username_service, $reply_markup, $caption, $invoice_id, $user_id = null, $image = 'images.jpg')
{
    ...
    $hubDeliveryResult = ['ok' => false, 'description' => 'no_delivery_attempted'];
    if ($STATUS_SEND_MESSAGE_PHOTO) {
        if ($panel_info['type'] == "WGDashboard") {
            ...
            $hubDeliveryResult = telegram('senddocument', [
                'chat_id' => $user_id,
            ...
            $hubDeliveryResult = telegram('sendphoto', [
                'chat_id' => $user_id,
        ...
        $hubDeliveryResult = sendmessage($user_id, $caption, $reply_markup, 'HTML');
    }
