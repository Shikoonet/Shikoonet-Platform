...
try {
    $baseRevenue = (int) db_query($pdo, "SELECT COALESCE(SUM(price_product),0) FROM invoice WHERE Status IN ('active','end_of_time','end_of_volume','sendedwarn','send_on_hold')")->fetchColumn();
    try {
        $manualSetting = db_fetch($pdo, "SELECT revenue_adjustment FROM setting LIMIT 1");
        $manualRevenueAdjustment = (int) ($manualSetting['revenue_adjustment'] ?? 0);
    } catch (Exception $e) {
        $manualRevenueAdjustment = 0;
    }
    $totalRevenue = $baseRevenue + $manualRevenueAdjustment;
...
<div class="stats fade-up">
    ...
    <div class="stat ok">
        ...
        <div class="stat-meta">
            <?php
            if ($manualRevenueAdjustment === 0) {
                echo $textbotlang['panel']['dashTotalSales'];
            } else {
                echo sprintf($textbotlang['panel']['dashAdjustedRevenueMeta'], number_format($manualRevenueAdjustment));
            }
            ?>
            <?php
            $adjCount = 0;
            try { $adjCount = (int) db_count($pdo, 'SELECT COUNT(*) FROM revenue_adjustment_log'); } catch (Exception $e) {}
            ?>
             · <a href="revenue_history.php" style="color:var(--ac);text-decoration:none;font-weight:600">
                <?= $adjCount > 0 ? 'View ' . $adjCount . ' adjustment' . ($adjCount === 1 ? '' : 's') . ' →' : 'View history →' ?>
             </a>
        </div>
    </div>
    <div class="stat warn">
