    ...
    try {
      ...
      setDone(
        messageKind === 'text'
          ...
          : `پست برای ${count(r.queued)} مشتری در صف قرار گرفت — یک نسخه هم همین حالا در تاپیک «گزارش تست» فرستاده شد تا ببینیدش.`,
      );
      setBody('');
      setPostLink('');
        ...
        >
          ...
          {messageKind === 'text' ? (
            ...
            <>
              ...
              <p className="muted">
                اول یک نسخه در تاپیک «گزارش تست» فرستاده می‌شود. اگر ربات به آن کانال دسترسی
                نداشته باشد، همین‌جا خطا می‌گیرید و هیچ‌چیز برای مشتری‌ها در صف نمی‌رود.
              </p>
            </>
          )}
