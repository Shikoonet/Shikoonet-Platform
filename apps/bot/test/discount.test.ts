...
describe('a plan that gives volume on its own (0082)', () => {
  /*
   * Sam, 2026-09-19: the percent is picked config by config on the dashboard
   * («ستاشون رو انتخاب می‌کنم … ۲۰٪ حجم اضافه»), and the words sit on the
   * button. The same pipe as a BONUS_PERCENT code, with nothing for the
   * customer to type. The fixture plan is 50 GB.
   */
  async function setPlanBonus(percent: number) {
    await db
      .prepare(`UPDATE product_plans SET bonus_percent = ?1 WHERE id = ?2`)
      .bind(percent, VIP_PLAN)
      .run();
  }
  afterEach(() => setPlanBonus(0));

  it('is said on the plan screen and lands on the order, price untouched', async () => {
    const { updateId, telegramId } = ids();
    const userId = await makeCustomer(telegramId);
    await setPlanBonus(20);

    // The button's own «+20٪ حجم هدیه» is menu.test.ts's; the seed sells one
    // plan per service, so `prd:` opens this screen without drawing a list.
