...
describe('the report templates are mirzabot’s, read from fa.php', () => {
  ...
  it('the renewal report is renewReportAdminFn with the previous service inserted above «نام محصول»', () => {
    ...
    // became. One line added to legacy's string; everything else is its.
    const PREVIOUS = '▫️سرویس قبلی : {previousPlan} — {previousVolume} GB — انقضا {previousExpiry}\n';
    ...
    const text = menu.renewalReport({
      ...
      previousPlan: 'یک ماهه', previousVolumeGb: 30, previousExpiresAtMs: AT,
    ...
    expect(text).toBe(
      sprintf('any', 'renewReportAdminFn', 123, 'sam', 'sam_1', 'آلمان', 'سه ماهه', 100, 90, '200,000', '50,000', STAMP)
        .replace('▫️نام محصول', `▫️سرویس قبلی : یک ماهه — 30 GB — انقضا ${STAMP}\n▫️نام محصول`),
    );
    // A migrated row that never recorded its plan or expiry: dashes, not «null».
    expect(menu.renewalReport({
      telegramId: 1, username: null, config: 'c', panel: 'p', previousPlan: null, previousVolumeGb: null,
      previousExpiresAtMs: null, plan: 'x', volumeGb: null, days: null, priceIrr: 0, balanceBeforeIrr: 0, atMs: AT,
    })).toContain('▫️سرویس قبلی : — — 0 GB — انقضا —\n');
  });
