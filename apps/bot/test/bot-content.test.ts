...
describe('the keyboard', () => {
  it('is production’s layout when nothing is customised', async () => {
    // `/start` refreshes the shop content and installs only the fixed home bar.
    // The shop layout itself is inline, opened by that bar and built here from
    // the same live content the handler uses.
    await start(await makeCustomer());
    const rows = menu.mainMenu(CUSTOMER);
