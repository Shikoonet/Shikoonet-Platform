...
function remove(id: string, identId: string) {
  return app.request(`/api/v1/accounts/${id}/identifier/${identId}`, { method: 'DELETE' }, envAs());
}
...
async function auditCount(action: string, entityId: string): Promise<number> {
  const row = await baseEnv.DB.prepare(
    `SELECT COUNT(*) AS n FROM audit_logs WHERE action = ?1 AND entity_id = ?2`,
  )
...
describe('an account with more than one number', () => {
  it('answers to a second account number without losing the first', async () => {
    ...
    expect((await add(id, { kind: 'ACCOUNT_HINT', value: '0031900001' })).status).toBe(200);
    expect((await add(id, { kind: 'CARD_LAST_FOUR', value: '6037' })).status).toBe(200);
  ...
  it('fills an empty column the way «create» would have, and only an empty one', async () => {
    ...
    expect((await add(id, { kind: 'CARD_LAST_FOUR', value: '1111' })).status).toBe(200);
    expect((await add(id, { kind: 'CARD_LAST_FOUR', value: '2222' })).status).toBe(200);
  ...
  it('adding the same number twice is one row, and one audit line', async () => {
    const id = await createAccount({ display_name: 'twice', account_hint: '5005' });
    expect((await add(id, { kind: 'ACCOUNT_HINT', value: '5006' })).status).toBe(200);
    const again = await add(id, { kind: 'ACCOUNT_HINT', value: '5006' });
    expect(again.status).toBe(200);
    expect(((await again.json()) as { added: boolean }).added).toBe(false);
    expect(await identifiersOf(id)).toHaveLength(2);
    expect(await auditCount('account.identifier_added', id)).toBe(1);
  });
  ...
  it('takes an extra number away, and the SMS for it no longer lands', async () => {
    ...
    expect((await add(id, { kind: 'ACCOUNT_HINT', value: '7002' })).status).toBe(200);
    ...
    expect(await auditCount('account.identifier_removed', id)).toBe(1);
    // Deleting it again finds nothing — and writes no second audit line.
    expect((await remove(id, extra.id)).status).toBe(404);
    expect(await auditCount('account.identifier_removed', id)).toBe(1);
  });
  ...
  it("cannot reach another account's row through this account's id", async () => {
    ...
    expect((await add(b, { kind: 'ACCOUNT_HINT', value: '6003' })).status).toBe(200);
