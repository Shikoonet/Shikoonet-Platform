...
| `INGEST_URL`                                          | **set by the deploy, not by hand**                 | Printed into the SMS-relay phone configuration. There is no fallback any more: the routes that need it answer 503 `INGEST_URL_MISSING`. Since the deploy writes it from the ingest application's own Coolify domain, typing it in the panel is unnecessary and a typo would be overwritten on the next deploy. An ingest with no domain REFUSES the deploy rather than guessing a hostname. It was «recommended» until 2026-08-29, and staging simply never got one: the box deployed green while `POST /devices` and both credential routes answered 503, so no device could be registered and no API key issued                                                                                                                                                                                                                                  |
...
| `SUPPORT_BOT_ADMIN_URL`, `SUPPORT_BOT_ADMIN_SECRET`   | no                                                 | The support bot's n8n admin webhook and the `x-shikoo-admin-key` it wants, for «محدودیت‌های ربات پشتیبانی» (#489). The key is a credential and is never rendered or logged. Unset, that page says the bot is not connected and nothing else notices |
...
| `ALERT_CHAT_ID`                                                            | fallback only                                      | Where a fault is announced when the shop has **no** `setting.Channel_Report` — since 2026-09-20 every error goes to the shop's own report group, «❌ گزارش خطا ها» topic, read from `settings` at send time on all three services; this variable covers only a database without that row. At most **one message an hour per event**, enforced by the `UNIQUE` on `bot_notifications.dedupe_key` rather than by a counter. Set it on all three services: an alert is a row in the queue, and the bot is what flushes it. On the bot it falls back to `REPORT_CHAT_ID`; on ingest and the dashboard it does not, because those have no report channel of their own. Unset means the events are still recorded in `app_events` and nobody is told. |
